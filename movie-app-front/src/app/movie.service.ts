...
export class MovieService {
  ...
  getFavourites():any{
    let search_url = `http://localhost:8080/movie-service/api/v1/movies`;
    return this.http.get(search_url);
  }
  addFavourites(newFav: favourites):any{
    let post_url = `http://localhost:8080/movie-service/api/v1/movie`;
    return this.http.post(post_url,newFav,httpOptions);
  }
  checkIfFav(id):any{
    let _url = `http://localhost:8080/movie-service/api/v1/movie/${id}`;
